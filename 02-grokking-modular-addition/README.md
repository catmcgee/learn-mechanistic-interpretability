...
![Vertical flow diagram of a transformer, read top to bottom. At the top, three coloured token squares labelled "a", "b", "=" captioned "input tokens". An arrow down to a rounded rectangle "embedding" with side-caption "W_E: lookup table, each token → 128-dim vector. Plus W_pos: per-position offset." Down to a dashed-border purple-tinted rectangle "residual stream — shape (3, 128): 3 positions × 128 features each". Down to a rounded "attention" rectangle (side-caption "4 heads. Moves information between positions. Writes output back into the residual stream."). Down to another dashed purple rectangle "residual stream (updated)". Down to "MLP" (side-caption "linear(128 → 512) → ReLU → linear(512 → 128). Per-position computation."). Down to "residual stream (updated again)". Down to "unembedding" (side-caption "W_U: linear(128 → 114). One score per possible answer."). Down to a final rounded rectangle "logits → softmax → predicted answer (read off at the = position)".](diagrams/transformer-architecture.png)
